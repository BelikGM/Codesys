...
1. **Библиотеки подключены в самом проекте**, а не только установлены в
   ...
   Часть 3. Открыть `Менеджер библиотек` в дереве и сверить состав со
   списком пунктов 27–31.
...
1. Драйвер `SPK_USB_Driver` должен быть установлен ЗАРАНЕЕ.
...
⚠️ При открытии может выскочить окно **«Обновление версий устройств
и библиотек»**. Здесь **ничего не обновлять**, если специально не
собирались: нажать «Не обновлять» / снять галочки. Обновление версии
таргета может потребовать пересборки всего дерева.
...
[SETUP.md](SETUP.md), пункты 27–31. Там же, в Части 3, описано, куда
именно каждая библиотека подключается: внешние — в двух местах,
встроенные — в одном, системные `Sys*` видны только после нажатия
«Дополнительно…» в окне добавления.
