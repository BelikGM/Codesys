...
рекомендована версия 3.3.1.40, а не 3.5.17.0, и эта кнопка может
затереть корректную рекомендацию.

Нажали случайно — не страшно. Закрыть проект **без сохранения** и
открыть заново; проверка результата — `F11`, ноль ошибок означает, что
откатывать нечего. Успели сохранить — вернуть файл из git:
`git checkout -- <файл проекта>`. Подробно — в [SETUP.md](SETUP.md),
раздел про этот диалог.
